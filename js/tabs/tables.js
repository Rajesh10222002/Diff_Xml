const TablesTab = {
    ...
    // Filter out tables that only differ by displaySource
    const realAddedTables = differences.tablesAdded.filter(table => {
        // If the table doesn't exist in schema1 at all, it's truly added
        return !schema1.tables[table];
    });

    const realRemovedTables = differences.tablesRemoved.filter(table => {
        // If the table doesn't exist in schema2 at all, it's truly removed
        return !schema2.tables[table];
    });

    // Filter changed tables to exclude those that only differ by joins or displaySource
    const realChangedTables = {};
    Object.keys(differences.tablesChanged).forEach(table => {
        ...
        const t1 = JSON.parse(JSON.stringify(table1));
        const t2 = JSON.parse(JSON.stringify(table2));
        
        // Remove joins and displaySource for comparison
        delete t1.attributes.joins;
        delete t2.attributes.joins;
        delete t1.attributes.displaySource;
        delete t2.attributes.displaySource;
        
        if (JSON.stringify(t1) !== JSON.stringify(t2)) {
            realChangedTables[table] = differences.tablesChanged[table];
        }
    });

    // Now use the filtered lists
    const filteredDifferences = {
        tablesAdded: realAddedTables,
        tablesRemoved: realRemovedTables,
        tablesChanged: realChangedTables,
        columnsAdded: differences.columnsAdded,
        columnsRemoved: differences.columnsRemoved,
        columnsChanged: differences.columnsChanged
    };

    // Rest of the function remains the same, but use filteredDifferences instead of differences
    const container = document.getElementById('tables-view-container');
    container.innerHTML = '';

    if (filteredDifferences.tablesAdded.length === 0 &&
        filteredDifferences.tablesRemoved.length === 0 &&
        Object.keys(filteredDifferences.tablesChanged).length === 0) {
   ...
   isTableChanged: function(table, schema1, schema2) {
    ...
    // Filter out displaySource from attributes comparison
    const filteredAttrs1 = Object.keys(table1.attributes).filter(attr => attr !== 'displaySource');
    const filteredAttrs2 = Object.keys(table2.attributes).filter(attr => attr !== 'displaySource');
    ...
    for (const attr of allAttrs) {
        // Skip joins and displaySource attributes
        if (attr === 'joins' || attr === 'displaySource') continue;
    ...
    createColumnDiffHtml: function(colName, col1, col2, changeType) {
    // Create a container for formula that will take full width
    let formulaHtml = '';
    let formulaToggle = '';
    
    // Only show formula toggle if there is a formula in either version
    if (col2?.formula || col1?.formula) {
        formulaToggle = `
            <div class="column-formula-toggle" onclick="TablesTab.toggleFormula(this)">
                <i class="fas fa-eye"></i> View Formula Changes
            </div>
        `;
        
        // If both versions have formulas and they're different
        if (col1?.formula && col2?.formula && col1.formula !== col2.formula) {
            formulaHtml = `
                <div class="column-formula" style="display: none;">
                    <div class="formula-comparison-container">
                        <div class="formula-old">
                            <div class="formula-header">
                                <i class="fas fa-file-alt danger-icon"></i> Old Formula
                            </div>
                            <div class="formula-content">
                                <pre>${this.formatFormula(col1.formula)}</pre>
                            </div>
                        </div>
                        <div class="formula-new">
                            <div class="formula-header">
                                <i class="fas fa-file-alt success-icon"></i> New Formula
                            </div>
                            <div class="formula-content">
                                <pre>${this.formatFormula(col2.formula)}</pre>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        // If only new version has formula
        else if (col2?.formula) {
            formulaHtml = `
                <div class="column-formula" style="display: none;">
                    <div class="formula-single">
                        <div class="formula-header">
                            <i class="fas fa-file-alt success-icon"></i> Added Formula
                        </div>
                        <div class="formula-content">
                            <pre>${this.formatFormula(col2.formula)}</pre>
                        </div>
        ...
        // If only old version has formula
        else if (col1?.formula) {
            formulaHtml = `
                <div class="column-formula" style="display: none;">
                    <div class="formula-single">
                        <div class="formula-header">
                            <i class="fas fa-file-alt danger-icon"></i> Removed Formula
                        </div>
                        <div class="formula-content">
                            <pre>${this.formatFormula(col1.formula)}</pre>
                        </div>
    ...
    switch(changeType) {
        ...
        case 'removed':
            ...
            break;
        case 'changed':
            statusIcon = 'fa-exchange-alt warning-icon';
            statusText = 'Changed';
            break;
        default:
    ...
    // For changed columns, show detailed attribute differences
    if (changeType === 'changed') {
        ...
        [...allAttrs].forEach(attr => {
            // Skip formula as we handle it separately
            ...
            if (val1 !== val2) {
                attributesHtml += `
                    <div class="column-attr changed">
                        <div class="attr-name">${attr}:</div>
                        <div class="attr-values">
                            <div class="attr-old-value">
                                <span class="value-label">Old:</span>
                                <span class="value">${val1 || '<em>null</em>'}</span>
                            </div>
                            <div class="attr-new-value">
                                <span class="value-label">New:</span>
                                <span class="value">${val2 || '<em>null</em>'}</span>
                            </div>
                        </div>
                    </div>
                `;
                ...
                attributesHtml += `
                    <div class="column-attr unchanged">
                        <div class="attr-name">${attr}:</div>
                        <div class="attr-value">${val1}</div>
                    </div>
                `;
            }
        });
        ...
        return `
            <div class="table-column ${changeType}">
    ...
    // For added/removed/unchanged columns
    const colData = col2 || col1;
    let attributesHtml = Object.entries(colData)
        .filter(([k]) => k !== 'formula')
        .map(([attr, val]) => `
            <div class="column-attr">
                <div class="attr-name">${attr}:</div>
